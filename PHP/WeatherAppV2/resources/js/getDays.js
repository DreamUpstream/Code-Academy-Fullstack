...
export function daysInfo(cityCode) {
	return fetch(`/weather/forecast/find/${cityCode}`)
		.then((r) => r.json())
		.then((info) => {
			let returner = {};
			let day = today();
			...
			for (let timeStamp of info.forecastTimestamps) {
				if (Object.keys(returner).length < 4) {
					if (
						timeStamp.forecastTimeUtc == now ||
						(timeStamp.forecastTimeUtc.split(" ")[1] == "12:00:00" && timeStamp.forecastTimeUtc.split(" ")[0] != day)
					) {
